...
public interface RecommendRepository extends JpaRepository<Recommend, Long> {
    ...
    Recommend findReRecommendByUserAndPosts(User user, Posts posts);

    // 게시글의 추천수 카운트
    Optional<Integer> countByPosts(Posts posts);

    // 내가 추천한 글 모두 조회
    @Query("SELECT r FROM Recommend r WHERE user = :user and type = :type ORDER BY r.id DESC")
    List<RecommendDto> findByUserAndRecommendPosts(@Param("user") User user, @Param("type") RecommendType type);

    Optional<Recommend> findByUserAndPostsAndType(User user, Posts posts, RecommendType type);
}
